...
<head>
    <title>ตรวจสอบยืมหนังสือเกินกำหนดในระบบ</title>
...
<body class="bg-gray-100">
    <div class="container mx-auto mt-10">
        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <h1 class="text-3xl font-bold mb-4">Upload Excel File ตรวจสอบยืมหนังสือเกินกำหนดในระบบ</h1>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['excel_file'])) {
        ...
        foreach ($data as $row) {
            $id_card = $row[0];
            $name_card = $row[1];
            $api_url = "https://imagesopac.sru.ac.th/v1/api/GetOverDuebyUser/$id_card";
        ...
        foreach ($results as $item) {
            ...
            foreach ($result as $subitem) {
                ...
                echo '<strong>วันที่ยืม:</strong> ' . $subitem['CHECKOUTDATE'] . '<br>';
                echo '<strong>วันกำหนดส่ง:</strong> ' . $subitem['DUEDATE'] . '<br>';
                echo '<strong>รวมวันทั้งหมด:</strong> ' . $subitem['DATEDIFF'] . ' วัน';
                echo '</li>';
            }
            echo '</ul>';
            echo '</td>';
